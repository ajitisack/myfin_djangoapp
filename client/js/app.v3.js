...
app.controller('myCtrl', function($scope, $filter, ExpenseFactory) {
    ...
    $scope.$watch('expenses', function (newValue, oldValue, scope) {
        summary = groupBySum($scope.expenses, "category", "amount")
        $scope.labels = getValues(summary, "category")
        $scope.data = getValues(summary, "amount")
    }, true);

    ExpenseFactory.query().$promise.then(function(result){
